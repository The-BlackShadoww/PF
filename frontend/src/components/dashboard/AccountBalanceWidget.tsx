...
export function AccountBalanceWidget() {
  ...
  if (isLoading) {
    return <div className="h-44 animate-pulse rounded-card bg-surface" />;
  }

  if (!summary || summary.initialBalanceCents === 0) {
    return (
      <div className="rounded-card border border-dashed border-muted bg-surface p-6">
  ...
  const dark = !summary.isLowBalance && !negative;
  ...
  return (
    ...
    >
      {(warning || negative) && (
        <div
          className={cn(
            "mb-4 flex gap-2 rounded-panel p-3 text-sm",
            negative
      ...
      <div
        className={cn(
          "mt-4 flex gap-5 text-xs",
          dark ? "text-canvas" : "text-muted",
        )}
      >
      ...
      {open && (
        <div className="mt-4 border-t border-current/10 pt-4">
          <SectorsBreakdown summary={summary} dark={dark} />
        </div>
      )}
    </section>
  );
