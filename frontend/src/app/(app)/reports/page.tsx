...
export default function ReportsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><PageHeader title="Reports" description="Download your financial data for any billing period." />
    <div className="rounded-card bg-surface p-6"><h2 className="text-base font-black text-ink">Configure report</h2><div className="mt-6"><label className="mb-2 block text-sm font-semibold text-muted">Billing period</label><div className="grid gap-3 sm:grid-cols-2">{([['From', startMonth, setStartMonth, startYear, setStartYear], ['To', endMonth, setEndMonth, endYear, setEndYear]] as const).map(([label, month, setMonth, year, setYear]) => <div key={label}><span className="mb-1 block text-xs text-muted">{label}</span><div className="flex gap-2"><select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="h-11 flex-1 rounded-panel border border-ink px-3 text-sm">{MONTH_NAMES.map((name, index) => <option key={name} value={index + 1}>{name}</option>)}</select><select value={year} onChange={(e) => setYear(Number(e.target.value))} className="h-11 w-24 rounded-panel border border-ink px-2 text-sm">{YEAR_OPTIONS.map((value) => <option key={value} value={value}>{value}</option>)}</select></div></div>)}</div>{dateRangeError && <p className="mt-2 flex items-center gap-1 text-xs font-semibold text-danger"><AlertCircle size={12} />{dateRangeError}</p>}</div><div className="mt-6 grid gap-3 sm:grid-cols-2">{([['csv', FileSpreadsheet, 'CSV'], ['pdf', FileText, 'PDF']] as const).map(([value, Icon, label]) => <button key={value} onClick={() => setFormat(value)} className={cn('rounded-card p-5 text-left', format === value ? 'bg-accent' : 'bg-canvas')}><Icon size={18}/><p className="mt-2 font-black">{label}</p></button>)}</div></div>
    <ReportPreviewCard preview={preview} isLoading={isPreviewLoading} {...params} />
    <Button onClick={() => !dateRangeError && download(params, format)} disabled={Boolean(dateRangeError || isDownloading)} className="w-full h-11 px-6"><Download size={16}/>{isDownloading ? 'Generating...' : `Download ${format.toUpperCase()} Report`}</Button>
    {downloadError && <div className="flex justify-between rounded-card bg-danger-surface p-4 text-white"><p>{downloadError}</p><button onClick={() => setDownloadError(null)}><X size={15}/></button></div>}
    <div className="rounded-card bg-surface p-6"><DownloadHistoryList history={history} onRedownload={(entry: DownloadHistoryEntry) => download(entry, entry.format)} onClear={clearHistory} isDownloading={isDownloading}/></div></div>;
}
